...
#define DM_MAX_DEVICES_NUM 1000
    ...
    void DeviceManage::onDeviceConnected(bool success, const QString &serial, const QString &deviceName,
                                         const QSize &size) {
    ...
    bool DeviceManage::isPortAvailable(const quint16 port) {
        QTcpServer server;
        const bool available = server.listen(QHostAddress::LocalHost, port);
        if (available) {
            server.close();
        }
        return available;
    ...
    quint16 DeviceManage::getFreePort() const {
        ...
        while (port < m_localPortStart + DM_MAX_DEVICES_NUM) {
            ...
            while (i.hasNext()) {
                i.next();
                if (const auto device = i.value(); device && device->isReversePort(port)) {
                    used = true;
                    break;
                }
            }
            if (!used && isPortAvailable(port)) {
                return port;
            }
            port++;
        }
